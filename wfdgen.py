class ValidateQSO:
    def __init__(self):
        ...
        # List of bands and their frequency range
    ...
    def validateband(self, freq): # Freq provided should fall into one of the bands, otherwise we will
        for currentband in self.bands.keys():
            if int(freq) >= self.bands[currentband][0] and int(freq) <= self.bands[currentband][1]:
               return True
            else:
                return False
    ...
    def check_line(self): # For each line, validate the available elements
        for line in range(len(self.rawlist)):
            if self.validatelength(self.rawlist[line]):
                ...
                if self.validateband(self.rawlist[line][0]):
...
class Score:
    def __init__(self, goodList):
        ...
        # Dictionary of bands and their frequency ranges
        self.bands = {'160m': [1800, 2000],
                      '80m': [3500, 4000],
                      '40m': [7000, 7300],
                      '20m': [14000, 14350],
    ...
    def gen_score(self):
        ...
        for line in self.goodList:
            ...
            self.multiplier.add(f'{self.findBand(int(line[0]))}+{line[5]}')
        ...
        print(str(total_points) + " * " + str(len(self.multiplier)) + " = " + str(total_points * len(self.multiplier)) + "\n")
...
class LogGen:
    def __init__(self, goodList):
        ...
        self.owncall = myConfig["owncall"]
        self.owncat = myConfig["owncat"]
        self.ownsec = myConfig["ownsec"]
        self.year = myConfig["year"]
        self.month = myConfig["month"]
...
validateMyLog.print_error_log()  # Print the errors

theScore = Score(validQsoList)  # Create object
theScore.gen_score()  # Generate score

thisThing = LogGen(validQsoList)
thisThing.build_log()
